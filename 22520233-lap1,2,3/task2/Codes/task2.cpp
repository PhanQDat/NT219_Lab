...
#define round_trip 1 // round encrypt&decrypt
#define round_trip_times 1.0 // round_trip_times = round_trip.0
...
// Set locale to support UTF-8
    ...
    while (true)
    {
        ...
        if (aes == 1)
        {
            cout << "Would you like input form screen or file?\n"
                 << "1. Screen\n"
                 << "2. File\n"
                 << "Please enter a number(1-2): ";
            int input;
            ...
            cout << "Total time for encryption: " << averageTime << " ms" << std::endl;
            if (argv[2] != NULL)
            {
                ...
                if (file.is_open())
                {
                    file << hex(cipher);
                    file.close();
                    cout << "Encrypted data saved to file successfully." << endl;
                }
                else
                {
                    cout << "Unable to save cipher file for writing." << endl;
                }
            }
        }
        else if (aes == 2)
        {
            ...
            auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();
            double averageTime = static_cast<double>(duration) / round_trip_times;
            cout << "Decrypted data: " << bytes_to_str(recovered) << endl;
            cout << "Total time for decryption: " << averageTime << " ms" << std::endl;
            return 0;
        }
        else
        {
