...
public:
    ...
    void generate_key_iv(char *key_file, char *iv_file) override
    {
        ...
        if (choice == 1)
        {
            ...
            // Save key to file
            StringSource(key, sizeof(key), true, new FileSink(key_file));
            cout << "key was saved to " << key_file << endl;
            ...
            // Save iv to file
            StringSource(iv, sizeof(iv), true, new FileSink(iv_file));
            cout << "iv was saved to " << iv_file << endl;
        }
        else if (choice == 2)
        {
            try
            {
                ...
                cin >> key;
                // Save key to file
                StringSource(key, sizeof(key), true, new HexEncoder(new FileSink(key_file)));
                cout << "key was saved to " << key_file << endl;
    ...
    void encrypt() override
    {
        ...
        StringSource ss(cipher, true, new HexEncoder(new StringSink(decoded)));
        auto end = std::chrono::high_resolution_clock::now();
        auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();
        double averageTime = static_cast<double>(duration) / round_trip_times;
        cout << "cipher: " << decoded << endl;
    ...
    void decrypt() override
    {
        ...
        try
        {
            ...
            // time start
            auto start = std::chrono::high_resolution_clock::now();
            StringSource ss(cipher, true, new HexDecoder(new StringSink(decoded)));
            for (int i = 0; i < round_trip; i++)
            {
                d.SetKeyWithIV(key, sizeof(key), iv);
                recovered.clear();
                StringSource ss(decoded, true, new StreamTransformationFilter(d, new StringSink(recovered)));
            }
            auto end = std::chrono::high_resolution_clock::now();
            auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();
            double averageTime = static_cast<double>(duration) / round_trip_times;
    ...
    void encrypt() override
    {
        ...
        try
        {
            ...
            for (int i = 0; i < round_trip; i++)
            {
                ...
                cipher.clear();
                StringSource ss1(plain, true, new AuthenticatedEncryptionFilter(e, new StringSink(cipher)));
            }
            StringSource(cipher, true, new HexEncoder(new StringSink(decoded)));
            auto end = std::chrono::high_resolution_clock::now();
            auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();
    ...
    void decrypt() override
    {
        ...
        try
        {
            ...
            for (int i = 0; i < round_trip; i++)
            {
                ...
                StringSource(decoded, true, new Redirector(df /*, PASS_EVERYTHING */)); // StringSource
            }
            auto end = std::chrono::high_resolution_clock::now();
            auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();
            double averageTime = static_cast<double>(duration) / round_trip_times;
...
int main(int argc, char **argv)
{
    ...
    std::set<string> modes = {"ECB", "ecb", "CBC", "cbc", "OFB", "ofb", "CFB", "cfb", "CTR", "ctr", "XTS", "xts", "CCM", "ccm", "GCM", "gcm"};
    string mode;
    int opt;
